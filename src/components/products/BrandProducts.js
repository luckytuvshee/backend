...
import { getBrandProducts, filterProduct } from "../../actions/product";
...
const BrandProducts = ({
  getBrandProducts,
  auth,
  product: { productBrands, brandProducts, loading, filterData },
  match,
  getLikedProducts,
  filterProduct,
}) => {
  ...
  const getBrandName = () => {
    for (let i = 0; i < productBrands.length; i++) {
      if (productBrands[i].id === parseInt(match.params.id)) {
        setBrandName(productBrands[i].brand_name);
        break;
      }
    }
  };
  ...
  useEffect(() => {
    getBrandProducts(match.params.id);
  }, [getBrandProducts, match.params.id]);

  useEffect(() => getBrandName());
